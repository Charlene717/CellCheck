...
CC_BarPlot <- function(Results.df, MetricsX = "Type", MetricsY = "Accuracy", MetricsG = "Tool") {
  ...
  ## Plot by group
  p <- ggplot(data = Results.df, aes(x = Results.df[,metricsX], y = Results.df[,metricsY],
                                     fill = Results.df[,metricsG]))+
  ...
  p2 <- p + scale_fill_brewer(palette = "Spectral")+ # scale_fill_manual(values = c("#999999", "#E69F00", "#56B4E9"))
        labs(#title = colnames(Results.df)[metricsY],  # Change title in ggplot
          #x = colnames(Results.df)[1],      # Change title of x axis in ggplot
          y = colnames(Results.df)[metricsY],       # Change title of y axis in ggplot
          fill= colnames(Results.df)[metricsG]     # Change legend title in ggplot
        )+
